...
df_jp_ctg = pd.read_csv("./csv_data/雇用_医療福祉_一人当たり賃金_全国_大分類.csv", encoding="shift_jis")
...
df_te_mean = df_jp_ind[df_jp_ind["年齢"]=="年齢計"]
df_te_mean = df_te_mean.rename(columns={"一人当たり賃金（万円）": "全国_一人当たり賃金（万円）"})
...
df_mean_line = pd.merge(df_te_mean, df_pref_mean, on="集計年")
...
df_bubble_mean = df_jp_ind[df_jp_ind["年齢"]!="年齢計"]
...
st.header("◆ 産業別の賃金推移")
year_list = df_jp_ctg["集計年"].unique()
...
df_mean_ctg = df_jp_ctg[df_jp_ctg["集計年"]==option_year]
max_x = df_mean_ctg[option_wage].max() + 50 # margin
fig = px.bar(
    df_mean_ctg,
    x=option_wage, 
    y="産業大分類名",
...
st.text("RESAS（地域経済分析システム）を加工して作成")